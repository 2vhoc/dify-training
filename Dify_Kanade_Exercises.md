...
[LLM: Trend Analysis (if batch)]            <-- Iterative (Pattern 4)
        |
[Template: Format Complete Response Package]
        |
[End: Resolution Recommendation + Trend Report]
```
...
```json
{
  ...
  "analysis": {
    ...
    "key_issues": ["3-day outage", "No communication"]
  },
  "sentiment_score": 2,
  "root_cause_hypothesis": [
    "Possible equipment failure",
    "Communication gap in support process"
  ],
  "similar_complaints": [
    {"id": "C009", "similarity": "High", "resolution": "Expedited repair + credit"}
  ],
  "recommended_response": {
    "immediate_action": "Manager callback within 2 hours",
    "technical_action": "Dispatch technician same-day",
    "communication": "Hourly updates until resolved",
    "compensation": "1 week service credit + priority support status"
  },
  "trend_insights": {
    "category_trend": "Service outage complaints up 15% this month",
    "regional_pattern": "Miyagi has highest complaint volume",
    "recommendation": "Review infrastructure in Miyagi region"
  }
}
```
...
- Click "Knowledge" → "Create Knowledge Base"
- Name: `Complaint Categories KB`
- Upload: `complaint_categories.md`
- Chunk size: 500 characters

**Step 2: Create Resolution Guidelines KB**
- Create another Knowledge Base
- Name: `Resolution Guidelines KB`
...
**Step 4: Configure Start Node**
- Input variables:
  - `customer_name` (Text) - Label: Customer Name
  - `complaint_channel` (Select) - Options: Phone, Email, Web, SNS
  - `complaint_text` (Paragraph) - Label: Complaint Description
  - `is_business_customer` (Select) - Options: Yes, No

**Step 5: Add Complaint Extraction LLM Node**
- Connect Start → LLM
- Prompt:
```
Analyze this customer complaint and extract key information:
...
Extract in JSON format:
{
  "emotion": "Very Angry/Angry/Frustrated/Disappointed/Neutral/Calm",
  "category": "Service Outage/Billing/Appointment/Equipment/Service Quality/Staff Conduct",
  "key_issues": [],
  "urgency_indicators": [],
  "business_impact": ""
}
...
**Step 6: Add Severity Scoring LLM Node**
...
**Step 7: Add IF/ELSE Severity Routing**
- Critical (8-10) → Immediate Escalation Path
- High (6-7) → Priority Response Path
- Medium (4-5) → Standard Response Path
- Low (1-3) → Queue Path

**Step 8: Add Parallel Processing Branch**
Create three parallel nodes:
- Branch 1: KB Retrieval - Get category definition and SLA
- Branch 2: LLM - Analyze root cause
- Branch 3: LLM - Find similar past complaints

**Step 9: Add Variable Aggregator**
- Combine all parallel outputs

**Step 10: Add Response Generation LLM Node**
- Generate customized response based on:
  - Emotion-appropriate tone
  - Category-specific resolution steps
  - Compensation guidelines (from KB)

**Step 11: Add Trend Analysis Code Node (Optional)**
- Python code to track complaint patterns
- Count by category, region, time period
...
Customer: {{customer_name}}
Channel: {{complaint_channel}}
Severity: {{severity_score}}/10 ({{severity_level}})
...
- Over 100,000 JPY AND useful life > 1 year → Use 42xxx codes
- Otherwise → Continue to Step 2

### Step 2: What is the primary purpose?
- IT/Software related → Use 43xxx codes
- Personnel related → Use 41301
- Facility related → Use 45xxx codes
- General operational → Use 44xxx codes

### Step 3: Specific Category Selection

#### Travel vs Training
- If attending as a PARTICIPANT → 44201 (Training)
- If traveling to GIVE presentation → 44101 (Travel)
- If both (e.g., conference) → Primary purpose determines code

#### Equipment vs Consumables
- Unit price > 100,000 JPY → 42101 (Equipment)
- Unit price < 10,000 JPY → 41101 (Consumables)
- Between 10,000-100,000 JPY → Consider durability

#### Maintenance vs Software
- For HARDWARE maintenance → 43301
- For SOFTWARE maintenance → 43201
- For COMBINED (hardware + software) → Split or use primary
...
```json
{
  "transaction_id": "T002",
  "input": {
    "description": "External training seminar registration fee",
    "department": "HR Development",
    "amount": 180000
  },
  "recommendation": {
    "primary_code": "44201",
    "code_name": "Training & Development",
    "confidence": "High"
  },
  "rationale": {
    "keywords_matched": ["training", "seminar", "registration"],
    "rules_applied": [
      "Purpose is employee skill development",
      "Not a travel expense (registration fee only)",
      "Not capital purchase (service, not asset)"
    ]
  },
  "similar_cases": [
    {"description": "Online course subscription", "code": "44201"},
    {"description": "Certification exam fee", "code": "44201"}
  ],
  "common_mistake_warning": "Note: If this includes travel costs, those should be coded separately as 44101",
  "validation_checklist": [
    "Confirm this is registration fee only (not travel)",
    "Verify department budget availability",
    "Check if pre-approval was obtained"
  ]
}
```
...
- Click "Knowledge" → "Create Knowledge Base"
- Name: `Accounting Codes KB`
- Upload: `accounting_codes.md`
- Chunk size: 300 characters (one code section per chunk)

**Step 2: Create Selection Rules KB**
- Create another Knowledge Base
- Name: `Code Selection Rules KB`
- Upload: `code_selection_rules.md`
- Chunk size: 400 characters

**Step 3: Test KB Retrieval**
- Test queries: `training expense`, `office supplies`, `software license`
...
**Step 5: Configure Start Node**
- Input variables:
  - `transaction_description` (Paragraph) - Label: Transaction Description
  - `department` (Text) - Label: Department
  - `amount` (Number) - Label: Amount (JPY)
  - `purpose` (Text) - Label: Purpose/Reason
...
{
  "expense_type": "",
  "is_capital_purchase": true/false,
  "primary_category": "",
  "keywords": [],
...
- Search `Accounting Codes KB`
- Use extracted keywords for search
- Output: `matching_codes`

**Step 8: Add Knowledge Retrieval - Rules KB**
- Search `Code Selection Rules KB`
- Use expense_type and category for search
- Output: `applicable_rules`

**Step 9: Add Code Matching LLM Node**
- Combine analysis + codes + rules
- Prompt:
```
Based on the transaction analysis and accounting rules, recommend the correct code:
...
**Step 11: Add Warning Check LLM Node**
- Check for common mistakes
- Add warnings if applicable (e.g., travel vs training confusion)
...
RECOMMENDED CODE: {{recommended_code}} - {{code_name}}
Confidence: {{confidence}}
